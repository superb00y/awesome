...
local createButtonpower = function(signal, icon, name)
	local widget = wibox.widget({
		{
			{
				{
...
return function()
	local widget = wibox.widget({
		...
		-- },
		{
			createButtonpower("poweroff", "*", "Poweroff"),
			createButtonpower("reboot", "#", "Reboot"),
			spacing = dpi(user.spacing),
			layout = wibox.layout.flex.horizontal,
		},
		{
			createButtonpower("lock", "@", "Lock"),
			createButtonpower("quit", "!", "Quit"),
			spacing = dpi(user.spacing),
			layout = wibox.layout.flex.horizontal,
		},
		layout = wibox.layout.fixed.vertical,
